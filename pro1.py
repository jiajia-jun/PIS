...
def stitch_images(images, mode, confidence):
    ...
    if status != cv2.Stitcher_OK and mode == cv2.Stitcher_PANORAMA:
...
交互式（不带任何参数）：
  直接运行 python stitch.py 会弹出文件选择框，手动选图拼接
    ...
    parser.add_argument("--confidence", type=float, default=0.6,   # 改为了 0.6
    ...
    try:
        ...
        if args.vertical:
            ...
            stitch_mode = cv2.Stitcher_SCAN
        else:
            print("横向拼接模式：使用球面投影（若失败将自动尝试平面拼接）")
